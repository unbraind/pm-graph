...
const EXTENSION_VERSION = "2026.9.19";
// ---------------------------------------------------------------------------
// Error contract
// ---------------------------------------------------------------------------
 ...
 */
function wrapCommandError(err, prefix, exitCode = EXIT_CODE.GENERIC_FAILURE) {
...
function getWorkspace(context) {
    if (context.pm_root)
        return workspaceFromPmRoot(context.pm_root);
 ...
 */
function resolveNeo4jModule() {
    const mod = requireNeo4j("neo4j-driver");
 ...
 */
function unwrapNeo4jInteger(value) {
 ...
 */
function assertPmTracker(pmRoot) {
    ...
    if (!hasSettingsFile && !hasSchemaDir) {
        throw new CommandError(`${pmRoot} is not a pm tracker (no settings.json file or schema/ directory present)`, EXIT_CODE.USAGE);
    }
    return pmRoot;
}
/**
...
function resolvePmRootForContext(context) {
    if (context.pm_root)
        return assertPmTracker(context.pm_root);
    try {
 ...
 */
export function parseAnalyticsFlags(args) {
    const flags = { json: false, includeClosed: false, format: "text", filter: [], positionals: [] };
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "--json") {
            ...
            const value = args[++i];
            if (value === undefined)
                throw new CommandError("--filter requires a value (type=... | status=...).", EXIT_CODE.USAGE);
...
export function parseNodeFilter(raw) {
    ...
    for (const term of raw) {
        ...
        if (eq <= 0) {
            throw new CommandError(`Invalid --filter "${term}" (expected key=value, e.g. type=Task or status=open,done).`, EXIT_CODE.USAGE);
        }
        const key = term.slice(0, eq).trim().toLowerCase();
        const valuePart = term.slice(eq + 1).trim();
        if (!NODE_FILTER_KEYS.has(key)) {
            throw new CommandError(`Invalid --filter key "${key}" (supported keys: type, status).`, EXIT_CODE.USAGE);
        }
 ...
 */
export function matchesNodeFilter(node, filter) {
    if (filter.length === 0)
        return true;
 ...
 */
function orderedNodeSet(rootId, rest = []) {
    const order = [];
    const seen = new Set();
    const add = (id) => {
        if (!seen.has(id)) {
 ...
 */
function collectBidirectionalEdges(paths) {
    const edgeKeys = [];
    const seen = new Set();
 ...
 */
function readNumberProperty(properties, key) {
...
export function explainItem(graph, id) {
    ...
    const mapNeighbors = (index) => [...index.entries()]
 ...
 */
function ambiguousItemIdError(label, input, matches) {
    const shown = matches.slice(0, 5);
    const more = matches.length > shown.length ? ` (+${matches.length - shown.length} more)` : "";
    return new CommandError(`${label} "${input}" is ambiguous in the workspace graph. Matches: ${shown.join(", ")}${more}. Use a longer prefix or the full id.`, EXIT_CODE.NOT_FOUND);
}
/**
...
function resolveItemIdOrThrow(itemIds, input, label) {
    ...
    throw new CommandError(`${label} "${requested}" was not found in the workspace graph.${hint}`, EXIT_CODE.NOT_FOUND);
}
/**
 ...
 */
async function loadShapedAnalytics(context) {
...
export function activate(api) {
    ...
    api.registerService("output_format", (ctx) => {
        ...
        if (ctx.command === "pm-graph export" &&
            ...
            const raw = result.__pmGraphRawOutput;
    ...
    api.registerCommand({
        ...
        run: async (context) => {
            ...
            if (hasHelpFlag(context)) {
                return {
                    usage: "pm pm-graph export [--format <cypher|mermaid|dot|json|graphml|plantuml>] [--output <file>] [--edges <deps|tags|all>] [--root <id> [--depth <n>]] [--filter key=value[,value]] [--include-closed] [--json]",
                    description: "Export the current workspace as a dependency and knowledge graph. Does not require Neo4j. By default emits the graph object (TOON, or JSON with --json). With --format, renders the graph into the given offline format on stdout (valid JSON for --format json). Shaping flags (--edges/--root/--depth/--filter/--include-closed) restrict the exported graph; when any shaping flag is given, closed/canceled items are excluded unless --include-closed is set.",
                    flags: {
            ...
            const shapingRequested = rawEdges !== undefined ||
                rawRoot !== undefined ||
                rawDepth !== undefined ||
            ...
            if (rawFormat === null || rawFormat.trim().length === 0) {
                throw new CommandError("--format requires a value (cypher | mermaid | dot | json | graphml | plantuml).", EXIT_CODE.USAGE);
            }
        ...
        run: async (context) => {
            if (hasHelpFlag(context)) {
                return {
                    ...
                    description: "Compute the shortest directed dependency path from <from> to <to> via BFS over STRUCTURAL edges. Item ids resolve by exact match, case-insensitive match, then unique prefix.",
                    flags: {
                        "--filter type=...|status=...": "Keep only PmItem nodes matching the given type/status (comma-list or repeat of same key = OR; different keys = AND)",
                        "--include-closed": "Include closed/canceled items",
                        "--json": "Output as JSON",
        ...
        run: async (context) => {
            if (hasHelpFlag(context)) {
                return {
                    ...
                    description: "Compute the longest chain of structural (blocking) dependencies through the workspace — the critical path. Reports the ordered id list and its length. Cycle-safe. With --format mermaid|graphml|dot, prints the critical-path chain as a diagram subgraph (chain nodes + connecting edges) for embedding in docs.",
                    flags: {
                        "--root <id>": "Restrict to the neighborhood of an item id",
                        "--depth <n>": "Max hop distance from --root",
                        "--filter type=...|status=...": "Keep only PmItem nodes matching the given type/status (comma-list or repeat of same key = OR; different keys = AND)",
    ...
    api.registerCommand({
        ...
        description: "Explain a single item offline: blockers, dependents, transitive impact, depth, and cycle participation.",
        run: async (context) => {
            if (hasHelpFlag(context)) {
                return {
                    usage: "pm pm-graph explain <id> [--filter type=...|status=...] [--include-closed] [--json]",
                    description: "Build an offline item-centric dependency report for one id over STRUCTURAL edges. Item ids resolve by exact match, case-insensitive match, then unique prefix.",
                    flags: {
                        "--filter type=...|status=...": "Keep only PmItem nodes matching the given type/status (comma-list or repeat of same key = OR; different keys = AND)",
                        "--include-closed": "Include closed/canceled items",
                        "--json": "Output as JSON",
