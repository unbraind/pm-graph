...
function graphFromItems(items, workspace) {
...
async function syncNeo4j(graph) {
    ...
    try {
        for (const node of graph.nodes) {
            await session.executeWrite((tx) => tx.run("MERGE (n:PmItem {id: $id}) SET n += $properties RETURN n.id", {
                id: node.id,
                properties: node.properties,
            }));
        }
        for (const relationship of graph.relationships) {
            await session.executeWrite((tx) => tx.run(`MATCH (from:PmItem {id: $from}), (to:PmItem {id: $to}) MERGE (from)-[r:${relationship.type}]->(to) SET r += $properties RETURN type(r)`, {
                from: relationship.from,
                to: relationship.to,
                properties: relationship.properties,
            }));
