    ...
    api.registerCommand({
        ...
        run: async (context) => {
            ...
            if (logicalDirection !== "downstream") {
                throw new CommandError(`--direction ${rawDirection} requires the canonical pm graph engine (pm-cli >= 2026.7.18); the installed pm-cli does not expose \`pm graph impact\`. Use --direction downstream (the default) or upgrade pm-cli.`, EXIT_CODE.USAGE);
            }
